...
# BEFORE RUNNING PLEASE UPDATE THESE VALUES [C.A. 2025-06]
sp_save_file = "path/to/your/save_file.csv"
cm_save_file = "path/to/another_save_file.csv"
orientation = "some_orientation_value"
...
files, training_intervals, mirror_name_list, all_mirrors = smu.get_training_data(
    d, "Mildura_Data_", time_to_remove_at_end=time_to_remove_at_end, helios=True
)
# training_intervals[0][1] -= np.datetime64(36,'h')

# orientation = [ [s[1]+s[2] for s in mirrors] for mirrors in mirror_name_list]
# orientation = [[elem.replace('_', '') for elem in s] for s in orientation]
...
ax.legend(fontsize=lgd_size, loc="center right", bbox_to_anchor=(1.25, 0.5))
