...
def find_truck_bounds(opt, verbose: bool = True):
    ...
    while True:
        ...
        if verbose:
            print(
                f"Trucks: {n_trucks}, Max cleanings: {max_cleans}, Total cleaning cost: ${total_cost:,.2f}"
            )
...
def optimize_periodic_schedule(opt, file: int = None, verbose: bool = True):
    ...
    for i, n_trucks in enumerate(Nt):
        ...
        for j, n_cleans in enumerate(nt_adjusted_nc):
            ...
            progress_bar.update(
                progress - progress_bar.n + (100 - remaining_progress)
            )  # Update relative to previous progress
...
def periodic_schedule_tcc(opt, n_trucks, n_cleans=None, verbose=True):
    ...
    for f in files:
        ...
        cleans[f] = simple_annual_cleaning_schedule(
            n_helios,
            n_trucks,
            n_cleans,
    ...
    for idx in np.arange(len(night_idx)):
        tcc_days[idx], ccl_days[idx], cdeg_days[idx] = _cleaning_cost(
            lost_power[: night_idx[idx]],
            alpha,
            opt,
            cleans[0][:, : night_idx[idx]],
            n_trucks,
        )
    ...
    results = {
        ...
        "soiling_induced_drops_below_upper_limit": np.sum(
            receiver_saturation["clean_field"] & ~receiver_saturation["soiled_field"]
        ),
        "day_tcc": tcc_days,
        "day_cdeg": cdeg_days,
    ...
    while True:
        ...
        for n_trucks in trucks_to_evaluate:
            if n_trucks not in results:
                ...
                schedule = rollout_heuristic_tcc(
                    opt, n_trucks, initial_arealoss=initial_arealoss, method="greedy"
                )
                total_cost = schedule["total_cleaning_costs"][file]
...
def rollout_heuristic_tcc(opt, n_trucks, initial_arealoss=None, method: str = "greedy"):
    ...
    def _clean_arealoss(area_loss, cleaning_schedule, cleaning_tech_curve=1.0):
        ...
        """
        area_loss[cleaning_schedule == 1] = area_loss[cleaning_schedule == 1] * (
            1.0 - cleaning_tech_curve
        )
        return area_loss
    ...
    def _dynamic_cleaning_schedule(
        sector_revenue, n_trucks, n_sectors_per_truck, n_sectors, n_day_horizon
    ):
                ...
                for sector, rev in sectors_to_try:
                    ...
                    if rev > 0:
                        ...
                        value = current_value + get_state_value(
                            day + 1, max_cleanings_per_day, new_mask
                        )
    ...
    def _greedy_cleaning_schedule(
        ...
        for _ in range(n_sectors * int(n_day_horizon)):
            valid_sector_revenue = (
                sector_revenue
                * available_heliostats[:, np.newaxis]
                * available_days[np.newaxis, :]
            )
            ...
            if np.any(valid_options):
                ...
                for future_day in range(cleaned_day + 1, int(n_day_horizon)):
                    ...
                    sector_revenue[cleaned_heliostat, future_day] = _sector_revenue(
                        ...
                        production_profit,
                        sector_cleaningcost,
                        existing_schedule=cleaning_schedule_horizon_day[
                            cleaned_heliostat, :
                        ].reshape(1, -1),
                    )

                available_heliostats[cleaned_heliostat] != np.sum(
    ...
    for f in files:
        ...
        opt.field_model.helios.optical_efficiency[f][
            np.isnan(opt.field_model.helios.optical_efficiency[f])
        ] = 0
        ...
        production_profit = (
            ...
            * (
                opt.plant.plant["electricity_price"] / 1e6
                - opt.plant.plant["plant_other_maintenance"] / 1e6
            )
        ...
        cleaning_schedule = np.zeros_like(
            soil_rate
        )  # Actual cleaning schedule after rollout heuristic

        for current_day in tqdm(
            np.unique(doy), desc=f"Simulating Day ({method.capitalize()})", leave=False
        ):  # Loop over each day of the year
            idx = _assign_indexing(
                current_day, doy, n_day_horizon
            )  # get the indices of the current day and the prediction horizon

            # Soilfactor Trajectories
            # Current status
            if current_day == 1:
                arealoss[:, idx["current"]] = np.cumsum(
                    copy.deepcopy(soil_rate[:, idx["current"]]), axis=1
                ) + initial_arealoss.reshape(-1, 1)
            else:
                arealoss[:, idx["current"]] = np.cumsum(
                    copy.deepcopy(soil_rate[:, idx["current"]]), axis=1
                ) + arealoss[:, idx["current"][0] - 1].reshape(-1, 1)

            # Soilrate Horizon
            horizon_soilrate = copy.deepcopy(
                soil_rate[:, idx["horizon"]]
            )  # Predictive area loss rate
            ...
            for clean_day in np.arange(
                int(n_day_horizon)
            ):  # Determine sector revenue for each possible cleaning day in the horizon
            ...
            arealoss[:, idx["cleaning"]] = _clean_arealoss(
                arealoss[:, idx["cleaning"]], cleaning_schedule[:, idx["cleaning"]]
            )
            ...
            if hasattr(opt, "record_daily_costs") and opt.record_daily_costs == 1:
                ...
                day_soilingfactor = _get_soilingfactor(
                    arealoss[:, : idx["cleaning"]],
                    incidence_factor[:, : idx["cleaning"]],
                )
                ...
                day_tcc, day_ccl, day_cdeg = _cleaning_cost(
                    day_lostpower,
                    production_profit,
                    opt,
    ...
    results["soiling_induced_off_times"] = np.sum(
        ~receiver_saturation["clean_field"] & receiver_saturation["soiled_field"]
    )
    results["soiling_induced_drops_below_upper_limit"] = np.sum(
        receiver_saturation["clean_field"] & ~receiver_saturation["soiled_field"]
    )
...
def _simulate_receiver_power(
    ...
    receiver_saturation = {}
    clean_receiver_power, clean_receiver_off, receiver_saturation["clean_field"] = (
        __receiver_operation(clean_sector_power, receiver_max, receiver_min)
    )
    dirty_receiver_power, dirty_receiver_off, receiver_saturation["soiled_field"] = (
        __receiver_operation(dirty_sector_power, receiver_max, receiver_min)
    )
...
def _cleaning_cost(lost_power, production_profit, opt, cleaning_schedule, n_trucks):
    ...
    cost_cleaning_var = (
        opt.field_model.helios.truck.consumable_costs["total"] * n_sectors_cleaned
    )  # [$/year]
...
def plot_cleaning_schedule(
    opt, results_schedule: dict, file: int = 0, save_path: str = None
) -> tuple:
    ...
    for col in df_cleaning_actions.columns:
        ...
        ax.scatter(
            df_cleaning_actions.index[mask],
            df_cleaning_actions[col][mask] * int(col + 1),
            marker=".",
            color="blue",
        )
...
def plot_soiled_optical_efficiency(
    opt, results_schedule: dict, file: int = 0, save_path: str = None
) -> tuple:
    ...
    sf = results_schedule["soiling_factor"][file]
    eta_clean = (
        opt.field_model.helios.optical_efficiency[file]
        * opt.field_model.helios.nominal_reflectance
    )
    ...
    ax.plot(
        opt.simulation_data.time[file],
        np.nanmean(eta_soiled, axis=0),
