...
from heliosoil.field_models import FieldModel, SimplifiedFieldModel, CentralTowerPlant
...
def optimize_periodic_schedule(opt, file: int = None, verbose: bool = True):
    ...
    total_iterations = sum(int(365 / (total_sectors / (n * sectors_per_truck))) - min_cleans + 1 for n in Nt)
    current_iteration = 0
...
def periodic_schedule_tcc(opt, n_trucks, n_cleans=None, verbose=True):
    ...
    """
    assert isinstance(opt, OptimizationProblem), "First input must be a heliosoil.cleaning_optimization.optimization_problem instance. "
    ...
    for f in files:
        ...
        cleans[f] = simple_annual_cleaning_schedule(n_helios, n_trucks, n_cleans, n_sectors_per_truck=field.helios.truck.n_sectors_per_truck)
    ...
    for fi in range(N_files):
        ...
        total_cleaning_cost[fi], C_cl[fi], C_deg[fi] = _cleaning_cost(lost_power, alpha, opt, cleans[f], n_trucks)
    ...
    for idx in np.arange(len(night_idx)):
        tcc_days[idx], ccl_days[idx], cdeg_days[idx] = _cleaning_cost(
            lost_power[: night_idx[idx]], alpha, opt, cleans[0][:, : night_idx[idx]], n_trucks
        )
...
def optimize_rollout_schedule(opt, file=0, verbose=True, max_trucks=20, initial_arealoss=None):
    ...
    return {"optimal_trucks": best_trucks, "optimal_results": best_results, "bounds_data": bounds_data, "all_results": results}
...
def rollout_heuristic_tcc(opt, n_trucks, initial_arealoss=None, method: str = "greedy"):
    ...
    def _dynamic_cleaning_schedule(sector_revenue, n_trucks, n_sectors_per_truck, n_sectors, n_day_horizon):
        ...
        def get_state_value(day, cleanings_left, cleaned_mask):
            ...
            if cleanings_left > 0:
                ...
                sector_revenues = [(s, sector_revenue[s, day]) for s in range(n_sectors) if not (cleaned_mask & (1 << s))]
                ...
                for sector, rev in sectors_to_try:
                    ...
                    if rev > 0:
                        ...
                        value = current_value + get_state_value(day + 1, max_cleanings_per_day, new_mask)
    ...
    def _greedy_cleaning_schedule(
        sector_revenue, soilrate, incidence_factor, clean_reflected_irradiance, n_trucks, n_sectors_per_truck, n_sectors, n_day_horizon
    ):
        ...
        for _ in range(n_sectors * int(n_day_horizon)):
            ...
            if np.any(valid_options):
                ...
                if available_days[0] is False:  # Break the greedy algoirmth once we have the schedule for the current day filled.
                    break
            else:
                break
    ...
    results = {
        "total_cleaning_costs": np.zeros(N_files),  # [$/yr] total_cleaning_cost: Total cleaning cost
    ...
    for f in files:
        ...
        clean_reflected_irradiance = (
            opt.simulation_data.dni[f][np.newaxis, :]
            * sector_area
            * opt.field_model.helios.optical_efficiency[f]
            * opt.field_model.helios.nominal_reflectance
        )
        production_profit = (
            opt.plant.plant["power_block_efficiency"]
            * (opt.plant.plant["electricity_price"] / 1e6 - opt.plant.plant["plant_other_maintenance"] / 1e6)
            * opt.simulation_data.dt[f]
            / 3600.0
        )
        ...
        for current_day in tqdm(np.unique(doy), desc=f"Simulating Day ({method.capitalize()})", leave=False):  # Loop over each day of the year
            ...
            if current_day == 1:
                ...
                arealoss[:, idx["current"]] = np.cumsum(copy.deepcopy(soil_rate[:, idx["current"]]), axis=1) + arealoss[
                    :, idx["current"][0] - 1
                ].reshape(-1, 1)

            # Soilrate Horizon
            horizon_soilrate = copy.deepcopy(soil_rate[:, idx["horizon"]])  # Predictive area loss rate
            horizon_soilrate[:, 0] += arealoss[
                :, idx["cleaning"]
            ]  # Add the last index of the current day i.e. posible 'cleaning' index arealoss to the first time period of the horizon
            ...
            for clean_day in np.arange(int(n_day_horizon)):  # Determine sector revenue for each possible cleaning day in the horizon
                ...
                sector_revenue[:, clean_day] = _sector_revenue(
                    horizon_soilrate,
            ...
            if method.lower() == "greedy":
                cleaning_schedule_horizon, _ = _greedy_cleaning_schedule(
                    sector_revenue,
                    horizon_soilrate,
                    incidence_factor[:, idx["horizon"]],
                    clean_reflected_irradiance[:, idx["horizon"]],
            ...
            if hasattr(opt, "record_daily_costs") and opt.record_daily_costs == 1:
                ...
                day_lostpower, _ = _simulate_receiver_power(
                    day_soilingfactor,
        ...
        lost_power, receiver_saturation = _simulate_receiver_power(
            soiling_factor,
            clean_reflected_irradiance,
            opt.plant.receiver["thermal_max"] * 1e6,
            opt.plant.receiver["thermal_min"] * 1e6,
        ...
        (results["total_cleaning_costs"][f], results["direct_cleaning_costs"][f], results["degradation_costs"][f]) = _cleaning_cost(
...
def _sector_revenue(
    soilrate, cleaning_schedule, incidence_factor, clean_reflected_irradiance, production_profit, sector_cleaningcost, existing_schedule=None
):
    """
...
def _simulate_receiver_power(soiling_factor, clean_sector_power, receiver_max, receiver_min, receiver_losses):
    ...
    receiver_saturation = {}
    clean_receiver_power, clean_receiver_off, receiver_saturation["clean_field"] = __receiver_operation(
        clean_sector_power, receiver_max, receiver_min
    )
    dirty_receiver_power, dirty_receiver_off, receiver_saturation["soiled_field"] = __receiver_operation(
        dirty_sector_power, receiver_max, receiver_min
    )

    lost_power = clean_receiver_power - dirty_receiver_power
    lost_power[dirty_receiver_off & (clean_receiver_off is False)] -= receiver_losses  # subtract losses so we lose only net power
...
def plot_optimization_results(results: dict, file: int = 0, save_path: str = None) -> tuple:
    ...
    for i, n_trucks in enumerate(trucks):
        ...
        if np.any(valid_costs):
            # Plot line
            ax.plot(
                np.array(cleans)[valid_costs], total_cleaning_cost[i, valid_costs] / 1e6, color=colors[i], label=f"{n_trucks} Trucks", linewidth=2
            )
            ...
            if not np.isnan(total_cleaning_cost[i, opt_idx]):
                ax.plot(cleans[opt_idx], total_cleaning_cost[i, opt_idx] / 1e6, marker="*", markersize=15, color=colors[i], linestyle="None")
    ...
    ax.text(text_x, text_y, textstr, transform=ax.transAxes, fontsize=10, verticalalignment="top", horizontalalignment="right", bbox=props)
...
def plot_soiling_factor(results_schedule: dict, file: int = 0, save_path: str = None) -> tuple:
    ...
    title = (
        f"Trucks {results_schedule['n_trucks']}, Avg. Cleanings {avg_cleans:.1f}\n"
...
def plot_soiled_optical_efficiency(opt, results_schedule: dict, file: int = 0, save_path: str = None) -> tuple:
    ...
    # Extract data
    sf = results_schedule["soiling_factor"][file]
    eta_clean = opt.field_model.helios.optical_efficiency[file] * opt.field_model.helios.nominal_reflectance
    ...
    ax.plot(opt.simulation_data.time[file], np.nanmean(eta_soiled, axis=0), label="Field Average (above stow angle only)", linewidth=2)

    # Add mean line
    ax.axhline(y=np.nanmean(eta_soiled), color="red", ls="--", label="Average")
    ...
    ax.set_title("Field Optical Efficiency with Soiling Effects", fontsize=14, pad=20)
    ax.xaxis.set_major_formatter(DateFormatter("%m"))
    ax.grid(True)
    ax.legend(fontsize=12)
    ax.set_ylim((0, 1))
